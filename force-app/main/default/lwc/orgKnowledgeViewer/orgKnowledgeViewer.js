...
export default class OrgKnowledgeViewer extends LightningElement {
    @api scanMode = DEFAULT_SCAN_MODE;
    ...
    get categoryResults() {
        return normalizeCategoryResults(
            getCategoryResults(
                this.analysisResult
            )
        );
    ...
    get dailyBrief() {
        ...
        return buildDailyBriefView(
            this.analysisResult
        );
    }

    get viewerMetrics() {
        return buildViewerMetrics(
            this.analysisResult,
            {
                inventoryCount:
                    this.objectInventory.length,
    ...
    get hasDeploymentBlockers() {
        return (
            this.deploymentBlockers.length >
            0
        );
    }
    ...
    get healthScoreExplanation() {
        return buildHealthScoreExplanation(
            this.analysisResult
        );
    }
    ...
    get hasHealthExplanationCategories() {
        return (
            this.healthExplanationCategories
                .length > 0
        );
    }
    ...
    get coverageLabel() {
        ...
        const failureText =
            failedCount > 0
                ? `; ${failedCount} object metadata requests failed`
                : '';
    ...
    get metadataCoverage() {
        ...
        const categories =
            this.normalizeMetadataCoverageCategories(
                rawCoverage
            );
        ...
        return {
            ...
            limitations:
                this.normalizeStringArray(
                    rawCoverage.limitations ||
                    rawCoverage.collectionLimitations
                ),
    ...
    normalizeMetadataCoverageCategories(
        rawCoverage = {}
    ) {
        const rawCategories =
            Array.isArray(
        ...
        return rawCategories.map(
            (
                ...
                const status =
                    category?.status ||
                    this.getCategoryCoverageStatus(
                        completionPercentage,
                        category
                    );
    ...
    get sharedIntelligenceStatus() {
        return buildSharedIntelligenceStatus(
            this.analysisResult
        );
    }
    ...
    async runAnalysis() {
        ...
        const priorSuccessfulResult =
            this.analysisResult;

        const priorRunnerResult =
            this.runnerResult;

        const previousSnapshot =
            priorSuccessfulResult?.success
                ? createTrendSnapshot(
        ...
        try {
            const result =
                await runOrgKnowledgeAnalysis({
                    scanMode:
                        this.normalizedScanMode,
            ...
            this.successMessage =
                `Org Knowledge analysis completed for ${this.organizationName} using the ${this.scanModeLabel}.${warningText}`;
        } catch (error) {
            /*
